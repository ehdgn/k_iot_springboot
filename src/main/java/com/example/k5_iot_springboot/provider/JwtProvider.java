...
public class JwtProvider {
    ...
    public JwtProvider(
            ...
            @Value("${jwt.refresh-expiration}") long jwtRefreshExpirationMs, // 필수
    ...
    *  >> 컨트롤러/필터에서 사용가능한 토큰인지 여부 확인 */
    public boolean isValidToken(String tokenWithoutBearer) {
            // 검증 - 서명 불일치, 변조, 포맷 이상, 만료(허용 오차 초과) 등 모든 예외는 catch로 전달 - false 반환 (parseSignedClaims가 처리)
        try {
